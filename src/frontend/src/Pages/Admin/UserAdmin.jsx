...
const UserAdmin = () => {
	...
	const handleBan = async (userId) => {
		const user = users.find(u => u._id === userId);
		if (user && user.role === 'admin') {
			toast.error('Không thể cấm tài khoản Admin!');
			return;
		}
		const result = await Swal.fire({
	...
	const handleUpdateRole = async (userId, currentRole) => {
		const user = users.find(u => u._id === userId);
		if (user && user.role === 'admin') {
			toast.error('Không thể cập nhật vai trò tài khoản Admin!');
			return;
		}
	...
	const handleBulkAction = async (action) => {
		...
		// Kiểm tra có admin trong danh sách không khi xóa
		if (action === 'delete') {
			const hasAdmin = users.filter(u => selectedUsers.includes(u._id)).some(u => u.role === 'admin');
			if (hasAdmin) {
				toast.error('Không thể xóa tài khoản Admin! Vui lòng bỏ chọn Admin trước khi xóa.');
				return;
			}
		}

		// Kiểm tra có admin trong danh sách không khi cấm
		if (action === 'ban') {
			const hasAdmin = users.filter(u => selectedUsers.includes(u._id)).some(u => u.role === 'admin');
			if (hasAdmin) {
				toast.error('Không thể cấm tài khoản Admin! Vui lòng bỏ chọn Admin trước khi cấm.');
				return;
			}
		}
	...
	return (
		<div className="">
			<h2 className="mb-2">Quản lý người dùng</h2>
